...
const run = async () => {
  await withTransaction(async (c) => {
    ...
    for (const b of BOARDS) {
      ...
      if (!memberKeys.includes("alex"))
        memberKeys.push("alex");
      ...
      for (let i = 0; i < b.tasks.length; i++) {
        ...
        const priority =
          PRIO_CYCLE[(i + b.title.length) % PRIO_CYCLE.length];

        const offset =
          DUE_CYCLE[(i + b.tasks.length) % DUE_CYCLE.length];
        ...
        const assigneeKey =
          i % 5 === 4
            ? null
            : assignPool[i % assignPool.length];
      ...
      const ownerName =
        USERS.find((u) => u.key === b.owner).name;
      ...
      const acts = [
        ...
        },
        {
          action: "task.moved",
          message: `${
            USERS.find((u) => u.key === memberKeys[1] || u.key === b.owner)?.name ||
            ownerName
          } moved "${b.tasks[Math.min(3, b.tasks.length - 1)]}" to Done`,
        },
      ];
    ...
    .then((taskTotal) => {
      ...
      console.log(
        "Login: alex@timetoprogram.com / Test@1234"
      );

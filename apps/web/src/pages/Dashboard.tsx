...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    const totalTasks = boards.reduce((sum, b) => sum + Number(b.task_count || 0), 0);
  ...
  const topBoards = useMemo(
    () =>
      [...boards].sort((a, b) => Number(b.task_count || 0) - Number(a.task_count || 0)).slice(0, 7),
    [boards],
  );

  const avgPerBoard = stats.total ? Math.round(stats.totalTasks / stats.total) : 0;
  const ownedPct = stats.total ? Math.round((stats.owned / stats.total) * 100) : 0;
  ...
  const trends = useMemo(() => {
    ...
    return {
      ...
      owned: boards.filter((b) => b.is_owner).map((b) => Number(b.task_count || 0)),
      shared: boards.filter((b) => !b.is_owner).map((b) => Number(b.task_count || 0)),
    };
  }, [boards]);

  const recentBoards = useMemo(
    () => [...boards].sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at)).slice(0, 4),
    [boards],
  );
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center gap-4 border-t pt-3.5 text-xs text-faint">
                      <span className="flex items-center gap-1.5 tabular">
                        <CheckSquare className="h-3.5 w-3.5" /> {b.task_count} tasks
                      </span>
                      <span className="flex items-center gap-1.5 tabular">
                        <Users className="h-3.5 w-3.5" /> {b.member_count}
                      </span>
                      <span className="ml-auto">{relativeTime(b.updated_at)}</span>
                    </div>
                  </Link>
                </motion.div>
              ))}
